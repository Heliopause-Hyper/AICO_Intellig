...
DEFAULT_OUT = os.path.join(REPO_ROOT, "results", "complexity_study", "shared_subspace_mismatch_large_v1.json")


@dataclass
class RidgeResidualTask:
    basis: np.ndarray
    hessian: np.ndarray
    ...
    def evaluate(self, x: np.ndarray) -> float:
        ...
        # A small ambient curvature makes the optimum unique in the full space.
        # This turns subspace rotation into a genuine approximation error floor.
...
def _make_task(
    ...
    return RidgeResidualTask(
        basis=basis,
        hessian=hessian,
        optimum_latent=optimum_latent,
        residual_weight=float(residual_weight),
        ambient_curvature=float(ambient_curvature) + float(residual_weight),
    )


def _coordinate_gradient(task: RidgeResidualTask, x: np.ndarray, eps: float) -> Tuple[np.ndarray, int]:
...
def _estimate_basis_from_history(
    tasks: Iterable[RidgeResidualTask],
    k: int,
    probe_points: int,
    fd_eps: float,
    rng: np.random.Generator,
...
def _run_condition(
    ...
    for family_seed in range(int(family_seeds)):
        rng = np.random.default_rng(
            ...
            + int(round(residual_weight * 1000))
            + family_seed
        )
        ...
        for task_idx, task in enumerate(test):
            seed_base = 950000 + family_seed * 1000 + task_idx * 10
            scratch_runs.append(
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Run shared-subspace mismatch sweep.")
    ap.add_argument("--out", default=DEFAULT_OUT)
    ap.add_argument("--d", type=int, default=64)
    ap.add_argument("--k", type=int, default=3)
    ap.add_argument("--family_seeds", type=int, default=16)
    ...
    ap.add_argument("--residual_weights", nargs="+", type=float, default=[0.0, 0.02, 0.05, 0.1])
    ...
    summary = {
        "config": {
            ...
            "residual_weights": [float(x) for x in args.residual_weights],
            "condition_number": float(args.condition_number),
            "ambient_curvature": float(args.ambient_curvature),
            "fd_eps": float(args.fd_eps),
    ...
    print("=" * 80)
    print("SHARED-SUBSPACE MISMATCH SWEEP")
    print("=" * 80)
    for row in rows[:8]:
        print(
            f"probe={row['probe_points_per_task']} angle={row['angle_deg']:.1f} residual={row['residual_weight']:.3f} "
