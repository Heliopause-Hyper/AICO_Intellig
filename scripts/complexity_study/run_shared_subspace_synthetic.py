...
DEFAULT_OUT = os.path.join(REPO_ROOT, "results", "complexity_study", "shared_subspace_synthetic.json")
...
class RidgeTask:
    ...
    def evaluate(self, x: np.ndarray) -> float:
        z = self.basis.T @ x
        diff = z - self.optimum_latent
        return 0.5 * float(diff.T @ self.hessian @ diff)
...
def _make_task(rng: np.random.Generator, basis: np.ndarray, condition_number: float) -> RidgeTask:
    k = int(basis.shape[1])
    ...
    return RidgeTask(basis=basis, hessian=hessian, optimum_latent=optimum_latent)


def _coordinate_gradient(task: RidgeTask, x: np.ndarray, eps: float) -> Tuple[np.ndarray, int]:
...
def _estimate_basis_from_history(
    ...
    for task in tasks:
        for _ in range(int(probe_points)):
            ...
            query_cost += cost
    if not grads:
        raise RuntimeError("history gradients are empty")
...
def _summarize_runs(runs: List[Dict[str, object]], thresholds: Iterable[float]) -> Dict[str, object]:
    ...
    for eps in thresholds:
        ...
        threshold_stats[str(eps)] = {
            "success_rate": float(len(success) / max(1, len(hits))),
            "mean_queries_if_hit": float(np.mean(success)) if success else None,
            "median_queries_if_hit": float(np.median(success)) if success else None,
        }
    summary["thresholds"] = threshold_stats
    return summary
...
def _dimension_sweep(
    ...
    for d in d_values:
        ...
        for family_seed in range(int(family_seeds)):
            rng = np.random.default_rng(1000 + 97 * int(d) + family_seed)
            u_true = _orthonormal_basis(rng, d=int(d), k=int(k))
            history = [_make_task(rng, u_true, condition_number=condition_number) for _ in range(int(history_tasks))]
            test = [_make_task(rng, u_true, condition_number=condition_number) for _ in range(int(test_tasks))]
            ...
            projector_errors.append(_operator_projector_error(u_true, u_est))
            history_costs.append(int(history_cost))

            full_basis = np.eye(int(d))
            for task_idx, task in enumerate(test):
                seed_base = 700000 + int(d) * 1000 + family_seed * 100 + task_idx
                scratch_runs.append(
                    _fd_descent_in_basis(
                        task,
                        full_basis,
                        budget=budget,
        ...
        out.append(
            {
                ...
                "projector_error_median": float(np.median(projector_errors)) if projector_errors else 0.0,
...
def _history_sweep(
    ...
    k: int,
    family_seeds: int,
    history_task_grid: Iterable[int],
    probe_point_grid: Iterable[int],
    test_tasks: int,
    budget: int,
    condition_number: float,
    fd_eps: float,
    ...
    out = []
    for history_tasks in history_task_grid:
        for probe_points in probe_point_grid:
            runs: List[Dict[str, object]] = []
            projector_errors: List[float] = []
            amortized_threshold_costs: Dict[str, List[float]] = {str(x): [] for x in thresholds}
            history_costs: List[int] = []

            for family_seed in range(int(family_seeds)):
                rng = np.random.default_rng(2000 + 131 * int(d) + history_tasks * 13 + probe_points * 7 + family_seed)
                u_true = _orthonormal_basis(rng, d=int(d), k=int(k))
                history = [_make_task(rng, u_true, condition_number=condition_number) for _ in range(int(history_tasks))]
                test = [_make_task(rng, u_true, condition_number=condition_number) for _ in range(int(test_tasks))]
                u_est, history_cost = _estimate_basis_from_history(
                ...
                for task_idx, task in enumerate(test):
                    run = _fd_descent_in_basis(
                        task,
                        u_est,
                        budget=budget,
                        rng=np.random.default_rng(900000 + family_seed * 100 + task_idx),
                        fd_eps=fd_eps,
                    )
                    runs.append(run)
                    for eps in thresholds:
                        hit = _first_hit(run["best_trace"], eps)
                        if hit is not None:
                            amortized_threshold_costs[str(eps)].append(
                                float(hit + history_cost / max(1, len(test)))
                            )
            ...
            row = {
                ...
                "history_query_cost_mean": float(np.mean(history_costs)) if history_costs else 0.0,
                "projector_error_mean": float(np.mean(projector_errors)) if projector_errors else 0.0,
                "estimated_subspace": _summarize_runs(runs, thresholds),
                "amortized_threshold_queries": {
                    eps: (float(np.mean(vals)) if vals else None)
                    for eps, vals in amortized_threshold_costs.items()
                },
            }
            out.append(row)
    return out
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Run synthetic shared-subspace complexity experiments.")
    ap.add_argument("--out", default=DEFAULT_OUT)
    ap.add_argument("--k", type=int, default=3)
    ap.add_argument("--d_values", nargs="+", type=int, default=[16, 64, 128])
    ap.add_argument("--family_seeds", type=int, default=5)
    ap.add_argument("--history_tasks", type=int, default=6)
    ap.add_argument("--test_tasks", type=int, default=10)
    ...
    ap.add_argument("--thresholds", nargs="+", type=float, default=[0.5, 0.1, 0.02])
    args = ap.parse_args()
    ...
    summary = {
        ...
        "dimension_sweep": _dimension_sweep(
        ...
        "history_sweep_d64": _history_sweep(
            d=64,
            k=int(args.k),
            family_seeds=int(args.family_seeds),
            history_task_grid=[2, 4, 8],
            probe_point_grid=[1, 2, 4],
            test_tasks=max(6, int(args.test_tasks) // 2),
            budget=int(args.budget),
            condition_number=float(args.condition_number),
            fd_eps=float(args.fd_eps),
            thresholds=args.thresholds,
    ...
    print("=" * 80)
    print("SYNTHETIC SHARED-SUBSPACE COMPLEXITY STUDY")
    print("=" * 80)
    for row in summary["dimension_sweep"]:
        print(
            f"d={row['d']:<3d} "
            f"proj_err={row['projector_error_mean']:.4f} "
            f"regret(scratch/oracle/est)="
            f"{row['scratch']['mean_simple_regret']:.4f}/"
            f"{row['oracle_subspace']['mean_simple_regret']:.4f}/"
            f"{row['estimated_subspace']['mean_simple_regret']:.4f}"
        )
    print("")
