...
# Load Pairwise Model Only (since static payload structure changed)
...
for func_name, func, pde_type in FUNCTIONS:
    for dim in [5, 10]:
        ...
        pairwise_decision = best_cand if best_gain > 0.0 else "SciPy-Nelder-Mead"
        
        results.append({
            "Function": f"{func_name}-{dim}D",
            "True_Best": true_best,
            "Our_Model_Prediction": pairwise_decision,
            "Predicted_Gain": round(best_gain, 3)
        })

df = pd.DataFrame(results)
print("\n--- ZERO-SHOT GENERALIZATION ON PURE MATH FUNCTIONS (BBOB-like) ---")
print(df.to_string())
